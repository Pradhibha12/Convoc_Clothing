...
public function dashboard()
{
    ...
    $topProducts = DB::table('order_items')
        ->select(
            'product_id',
            DB::raw('SUM(quantity) as total_quantity'),
            DB::raw('SUM(COALESCE(CAST(REPLACE(price, ",", "") AS DECIMAL(10,2)), 0) * quantity - COALESCE(discounted_amount, 0)) as calculated_total_sales')
        )
        ->groupBy('product_id')
        ->orderByDesc('total_quantity')
        ->get();
    ...
    $topProducts = $topProducts->map(function ($item) use ($productDetails) {
        ...
        return (object) [
            ...
            'total_quantity' => $item->total_quantity,
            'total_sales'    => round(max(0, $item->calculated_total_sales), 2),
        ];
    });
    ...
    public function category_delete($id)
{
    ...
    function attribute_store(Request $request)
    {
        ...
        $data['name'] = $request->name;
        $data['slug'] = app(CommonController::class)->unique_slug($request->name, 'attributes');
    ...
    function attribute_update(Request $request, $id)
    {
        ...
        $data['name'] = $request->name;
        $data['slug'] = app(CommonController::class)->unique_slug($request->name, 'attributes', 'slug', $id);
        $data['input_value'] = $request->input_value;
        $data['updated_at'] = date('Y-m-d H:i:s');
    ...
    function product_add()
    {
        ...
        if ($referer && str_contains($referer, route('admin.products'))) {
            session(['product_add_referrer' => $referer]);
        }
    ...
    function product_store(Request $request, $action_type = '')
    {
        ...
        if ($action_type == 'quick') {
            ...
            $request->validate([
                ...
                'status' => 'nullable|in:1,0',
                'title' => 'required|max:255',
                'label' => 'nullable|in:top-seller,best-seller,featured,trendy,new-arrival,hot,exclusive,limited-edition,bestselling,customer-favorite',
                'quality_label' => 'nullable|in:certified,premium,authentic,handmade,organic,sustainable',
                'price' => 'required|numeric',
            ...
            $data['summary'] = $request->summary;
            $data['description'] = $request->description;
            $data['unit'] = $request->unit ?? 'nos';
            $data['total_stock'] = $request->total_stock ?? 0;
            $data['price'] = $request->price;
            $data['created_at'] = date('Y-m-d H:i:s');
            ...
            if ($request->hasFile('thumbnail')) {
                foreach ($request->file('thumbnail') as $key => $image) {
                    $image_path = app(CommonController::class)->upload($image, 'uploads/product/thumbnail/' . nice_file_name($key.'-'.time(), $image->extension()), 800);
                    $image_path = str_replace('\\', '', $image_path);
                    array_push($product_image, $image_path);

                    if (isset($request->new_image_attributes[$key])) {
                        $image_attributes[$image_path] = $request->new_image_attributes[$key];
                    }
                }
            ...
            $data['image_attributes'] = json_encode($image_attributes);
        }
        ...
        if ($product_id > 0 && $action_type != 'quick') {
            ...
            //Product discounts
            if ($request->discount_period && $request->filled('discount_value')) {
                $product_discount['product_id'] = $product_id;
                $product_discount['discount_type'] = $request->discount_type ?? 'flat';
                $product_discount['discount_value'] = $request->discount_value;
                $product_discount['status'] = $request->discount_status ?? 0;
        ...
        $referrer = session('product_add_referrer') ?? route('admin.products');
        session()->forget('product_add_referrer');
        return redirect($referrer)->with('success', get_phrase('Product added successfully'));
    }
    ...
    function product_edit($id)
    {
        $referer = request()->headers->get('referer');
        if ($referer && str_contains($referer, route('admin.products'))) {
            session(['product_edit_referrer' => $referer]);
        }
        $page_data['product_categories'] = Category::get();
    ...
    function product_update(Request $request, $id)
    {
        ...
        $product_image_raw = json_decode(Product::where('id', $id)->pluck('thumbnail')->toArray()[0])??[];
        $product_image = [];
        foreach ($product_image_raw as $img) {
            $product_image[] = str_replace('\\', '', $img);
        }

        $image_attributes = $request->image_attributes ?? [];
        $cleaned_image_attributes = [];
        foreach ($image_attributes as $k => $v) {
            $cleaned_image_attributes[str_replace('\\', '', $k)] = $v;
        }
        ...
        if ($request->hasFile('thumbnail')) {
            foreach ($request->file('thumbnail') as $key => $image) {
               ...
               if (isset($request->new_image_attributes[$key])) {
                   $cleaned_image_attributes[$image_path] = $request->new_image_attributes[$key];
               }
            }
            $data['thumbnail'] = json_encode($product_image);
        }else{
        ...
        $data['image_attributes'] = json_encode($cleaned_image_attributes);

        Product::where('id', $id)->update($data);


        //Product attributes
        if ($request->has('visible_attribute_types')) {
            foreach ($request->visible_attribute_types as $attribute_type_id) {
                Product_attribute::where('product_id', $id)->where('attribute_type_id', $attribute_type_id)->delete();

                if (isset($request->product_attributes[$attribute_type_id])) {
                    foreach ($request->product_attributes[$attribute_type_id] as $attribute_id => $val) {
                        if (!empty($val)) {
                            $product_attribute = [
                                'product_id' => $id,
        ...
        $referrer = session('product_edit_referrer') ?? route('admin.products');
        session()->forget('product_edit_referrer');
        return redirect($referrer)->with('success', get_phrase('Product updated successfully'));
    }

    function product_image_delete(\Illuminate\Http\Request $request, $id)
    {
        ...
        $imageArray = is_array(json_decode($product->thumbnail, true)) ? json_decode($product->thumbnail, true) : [];
        $updatedArray = [];

        foreach ($imageArray as $img) {
            if (basename($img) === basename($image) || $img === $image) {
                remove_file($img);
            } else {
                $updatedArray[] = $img;
            }
        ...
        $product->thumbnail = json_encode(array_values($updatedArray));
        $product->save();

        return response()->json(['status' => true, 'message' => get_phrase('Image removed successfully')]);
    }

    function product_banner_delete($id)
    {
        $product = Product::find($id);
        if (!$product) {
            return response()->json(['status' => false, 'message' => 'Product not found']);
        ...
        if (!empty($product->banner)) {
            remove_file($product->banner);
            $product->banner = null;
            $product->save();
        }

        return response()->json(['status' => true, 'message' => get_phrase('Banner image removed successfully')]);
    }
    ...
    public function product_duplicate($id)
    {
        ...
        $new_title = $product->title . ' (Copy)';
        $new_product = $product->replicate();
        $new_product->title = $new_title;
        $new_product->slug = app(CommonController::class)->unique_slug($new_title, 'products');
        $new_product->code = app(CommonController::class)->unique_code('products', 'code');
        ...
        foreach ($attributes as $attr) {
            ...
            $new_attr->product_id = $new_product_id;
            $new_attr->created_at = date('Y-m-d H:i:s');
            $new_attr->updated_at = date('Y-m-d H:i:s');
            $new_attr->save();
        }

        // Duplicate product discount
        $discount = Product_discount::where('product_id', $id)->first();
        if ($discount) {
            $new_discount = $discount->replicate();
            $new_discount->product_id = $new_product_id;
            $new_discount->created_at = date('Y-m-d H:i:s');
            $new_discount->updated_at = date('Y-m-d H:i:s');
            $new_discount->save();
        }

        // Duplicate SEO field if exists
        $seo_field = Seo_field::where('item_table', 'products')->where('item_id', $id)->first();
        if ($seo_field) {
            $new_seo = $seo_field->replicate();
            $new_seo->item_id = $new_product_id;
            $new_seo->created_at = date('Y-m-d H:i:s');
            $new_seo->updated_at = date('Y-m-d H:i:s');
            $new_seo->save();
        }

        // Duplicate event relations
        if ($product->events()->exists()) {
            $new_product->events()->sync($product->events->pluck('id'));
        }

        return redirect()->back()->with('success', get_phrase('Product duplicated successfully'));
    }
    ...
    function system_settings_update(Request $request)
    {
        ...
        $request->validate([
            ...
            'sms_gateway' => 'required|in:mock,firebase,custom,whatsapp',
            'custom_sms_url' => 'nullable|max:2000',
            'whatsapp_api_url' => 'nullable|max:255',
            'whatsapp_instance' => 'nullable|max:255',
            'whatsapp_api_key' => 'nullable|max:255',
            'firebase_api_key' => 'nullable|max:255',
            'firebase_auth_domain' => 'nullable|max:255',
            'firebase_project_id' => 'nullable|max:255',
            'firebase_storage_bucket' => 'nullable|max:255',
            'firebase_messaging_sender_id' => 'nullable|max:255',
            'firebase_app_id' => 'nullable|max:255'
    ...
    {
        $page_data['page_title'] = get_phrase('Header Menu Manager');
        $items_json = get_frontend_settings('header_menu_items');
        $page_data['menu_items'] = $items_json ? json_decode($items_json, true) : [
            ['title' => 'Home', 'url' => '/', 'type' => 'route', 'route' => 'home', 'target' => '_self'],
            ['title' => 'Categories', 'url' => '#', 'type' => 'category_dropdown', 'route' => '', 'target' => '_self'],
            ['title' => 'Store', 'url' => '/store', 'type' => 'route', 'route' => 'store', 'target' => '_self'],
            ['title' => 'Events', 'url' => '/events', 'type' => 'route', 'route' => 'events', 'target' => '_self'],
            ['title' => 'Blog', 'url' => '/blog', 'type' => 'route', 'route' => 'blog', 'target' => '_self'],
        ];
        return view('admin.settings.menu_settings', $page_data);
    }

    public function menu_settings_update(Request $request)
    {
        $menu_items = [];
        ...
        $json_data = json_encode($menu_items);
        if (Frontend_setting::where('type', 'header_menu_items')->count() == 0) {
            Frontend_setting::insert(['type' => 'header_menu_items', 'description' => $json_data]);
        } else {
            Frontend_setting::where('type', 'header_menu_items')->update(['description' => $json_data]);
        }

        return redirect()->back()->with('success', get_phrase('Header menu items updated successfully'));
    }

    public function footer_settings()
    {
        $page_data['page_title'] = get_phrase('Footer Settings & Links');
        
        $quick_links_json = get_frontend_settings('footer_quick_links');
        $page_data['quick_links'] = $quick_links_json ? json_decode($quick_links_json, true) : [
            ['title' => 'Home', 'url' => '/'],
            ['title' => 'Blog', 'url' => '/blog'],
        ...
        $support_links_json = get_frontend_settings('footer_support_links');
        $page_data['support_links'] = $support_links_json ? json_decode($support_links_json, true) : [
            ['title' => 'About Us', 'url' => '/about-us'],
            ['title' => 'Privacy Policy', 'url' => '/privacy-policy'],
            ['title' => 'Refund Policy', 'url' => '/refund-policy'],
            ['title' => 'Terms and condition', 'url' => '/terms-and-conditions'],
        ];

        $page_data['footer_about_text'] = get_frontend_settings('footer_about_text') ?? 'Convoc was born out of a simple yet profound idea – to provide high-quality promotional T-shirts at the best price.';
        $page_data['footer_copyright_text'] = get_frontend_settings('footer_copyright_text') ?? 'Copyright © ' . date('Y') . ' All Rights Reserved. Designed with ❤️ by Wspl Hosting Solution (OPC) Private Limited';
        $page_data['footer_quick_links_title'] = get_frontend_settings('footer_quick_links_title') ?? 'Quick Links';
        $page_data['footer_top_categories_title'] = get_frontend_settings('footer_top_categories_title') ?? 'Top Categories';
        $page_data['footer_support_title'] = get_frontend_settings('footer_support_title') ?? 'Support';
        $page_data['footer_contact_title'] = get_frontend_settings('footer_contact_title') ?? 'Contact Us';

        return view('admin.settings.footer_settings', $page_data);
    }

    public function footer_settings_update(Request $request)
    {
        $settings = [
            'footer_about_text' => $request->footer_about_text,
            'footer_copyright_text' => $request->footer_copyright_text,
            'footer_quick_links_title' => $request->footer_quick_links_title ?? 'Quick Links',
            'footer_top_categories_title' => $request->footer_top_categories_title ?? 'Top Categories',
            'footer_support_title' => $request->footer_support_title ?? 'Support',
            'footer_contact_title' => $request->footer_contact_title ?? 'Contact Us',
        ];
        ...
        $settings['footer_quick_links'] = json_encode($quick_links);

        $support_links = [];
        if ($request->has('support_links') && is_array($request->support_links)) {
            foreach ($request->support_links as $link) {
                if (!empty($link['title'])) {
                    $support_links[] = [
                        'title' => $link['title'],
                        'url' => $link['url'] ?? '#'
        ...
        $settings['footer_support_links'] = json_encode($support_links);
        ...
        return redirect()->back()->with('success', get_phrase('Footer settings updated successfully'));
    }
