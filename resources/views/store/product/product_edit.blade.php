            ...
            <form action="{{ route('vendor.product.update', ['id' => $product->id]) }}" method="post" enctype="multipart/form-data">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-lg-6 d-flex justify-content-end flex-wrap gap-2">
                        ...
                        <a href="{{ route('vendor.product.duplicate', ['id' => $product->id]) }}" class="btn ol-btn-light h-45 d-flex align-items-center cg-10px">
                            <span class="fi-rr-copy"></span>
                            <span>{{ get_phrase('Duplicate product') }}</span>
                        </a>
                        <a href="{{ session('product_edit_referrer') ?? route('vendor.products') }}" class="btn ol-btn-outline-secondary h-45 d-flex align-items-center cg-10px">
                        ...
                        <div class="ol-card">
                            ...
                            <div class="ol-card-body p-3 mb-5">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="number" min="0" step="0.01" value="{{ $product->discount ? $product->discount->discount_value : '' }}" name="discount_value" class="form-control ol-form-control" id="discount_value" placeholder="{{ get_phrase('Enter amount of discount') }}" aria-label="{{ get_phrase('Enter amount of discount') }}">
                                </div>
                                    ...
                                    <div class="position-relative position-relative">
                                        <input type="text" value="{{ 
                                                ...
                                                ($product->discount && $product->discount->end_date)
                                                    ...
                                                    : date('m/d/Y')
                                            }}" name="discount_period" class="form-control ol-form-control daterangepicker w-100" id="discount_period" placeholder="{{ get_phrase('Select date range of discount period') }}" aria-label="{{ get_phrase('Select date range of discount period') }}">
                                    </div>
                                </div>
                        ...
                        <div class="ol-card">
                            ...
                            <div class="ol-card-body p-3 mb-5">
                                <div class="mb-3">
                                    ...
                                    <div class="d-flex flex-wrap gap-2 mb-3" id="image-container">
                                        @if(!empty($product->thumbnail) && is_array(json_decode($product->thumbnail, true)))
                                            @foreach(json_decode($product->thumbnail, true) as $key => $image)
                                                ...
                                                @endphp
                                                <div class="position-relative border rounded p-1 shadow-sm" id="gallery-image-{{ $key }}" style="width: 100px; height: 100px;">
                                    ...
                                    <input type="file" name="thumbnail[]" class="form-control ol-form-control" id="thumbnail" accept="image/*" multiple onchange="previewGalleryImages(this, 'new-gallery-previews')">
            ...
            if (input.files && input.files.length > 0) {
                Array.from(input.files).forEach((file, index) => {
                    ...
                    reader.onload = function(e) {
                        ...
                        div.innerHTML = `<img src="${e.target.result}" class="w-100 h-100 rounded object-fit-cover"><span class="badge bg-dark position-absolute bottom-0 start-0 m-1" style="font-size:10px;">New ${index + 1}</span>`;
