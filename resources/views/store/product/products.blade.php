    ...
    <div class="ol-card">
        ...
        <div class="ol-card-body p-3 mb-5">
            <div class="row mb-4 mt-3">
                <div class="col-md-7 d-flex align-items-center flex-wrap gap-3">
                    <a href="{{ route('vendor.product.add') }}" class="btn ol-btn-outline-secondary d-flex align-items-center cg-10px">
                    ...
                    <div class="custom-dropdown dropdown-filter @if (!isset($_GET) || (isset($_GET) && count($_GET) == 0))  @endif">
                        ...
                        <ul class="dropdown-list w-250px">
                            <li>
                                <form id="filter-dropdown" action="{{ route('vendor.products') }}" method="get">
                    ...
                    @php
                        ...
                        if (request()->filled('category') && request('category') != 'all') {
                            ...
                            $active_filters[] = [
                                'label' => get_phrase('Category'),
                                'value' => $cat ? $cat->title : request('category'),
                                'key' => 'category'
                            ];
                        }
                        if (request()->filled('brand') && request('brand') != 'all') {
                            $active_filters[] = [
                                'label' => get_phrase('Brand'),
                                'value' => request('brand'),
                                'key' => 'brand'
                            ];
                        }
                        ...
                        <div class="d-flex flex-wrap gap-2 align-items-center ms-2">
                            @foreach ($active_filters as $filter)
                                @php
                                    ...
                                    $removeUrl = route('vendor.products', $params);
                                @endphp
                                <span class="badge bg-primary text-white d-flex align-items-center gap-2 py-2 px-3 fs-12px rounded-pill shadow-sm" style="font-weight: 500;">
                                    <span>{{ $filter['label'] }}: <strong>{{ $filter['value'] }}</strong></span>
                                    <a href="{{ $removeUrl }}" class="text-white ms-1" style="line-height: 1;"><i class="fi-rr-cross-small" style="font-size: 10px;"></i></a>
                                </span>
                            @endforeach
                            <a href="{{ route('vendor.products') }}" class="btn btn-sm btn-outline-danger py-1 px-2 rounded-pill fs-11px" data-bs-toggle="tooltip" title="{{ get_phrase('Clear All Filters') }}">
                ...
                <div class="col-md-5 mt-md-0 mt-3">
                    <form action="{{ route('vendor.products') }}" method="get">
                ...
                <div class="table-responsive overflow-auto">
                    <table class="table print-table">
                        ...
                        <tbody>
                            ...
                            {{print_r($product->discount_value);}}
                                @php
                                    $thumbnails = json_decode($product->thumbnail, true);
                                    $firstImage = isset($thumbnails[0]) ? str_replace('\\', '', $thumbnails[0]) : null;
                                @endphp
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex align-items-center gap-2">
                                            ...
                                            <a href="javascript:;" onclick="downloadBarcode('{{ $barcode_image }}', '{{ $product->code }}')" class="btn action-label download-btn" data-bs-toggle="tooltip" title="{{ get_phrase('Download') }}">
                                    ...
                                    <td>
                                        <div class="print-d-none d-flex flex-wrap align-items-center gap-2 row-gap-2">
                                            <a href="{{ route('vendor.product.edit', ['id' => $product->id]) }}" data-bs-toggle="tooltip" title="{{ get_phrase('Edit') }}" class="btn btn-primary-light btn-icon"><i class="fi-rr-pencil"></i></a>
                                            <a href="{{ route('vendor.product.duplicate', ['id' => $product->id]) }}" data-bs-toggle="tooltip" title="{{ get_phrase('Duplicate') }}" class="btn btn-secondary-light btn-icon"><i class="fi-rr-copy"></i></a>
                                            <a href="javascript:;" onclick="confirmModal('{{ route('vendor.product.delete', ['id' => $product->id]) }}')" data-bs-toggle="tooltip" title="{{ get_phrase('Delete') }}" class="btn btn-danger-light btn-icon"><i class="fi-rr-trash"></i></a>
                                        </div>
                                    </td>
                                </tr>
                            @endforeach
