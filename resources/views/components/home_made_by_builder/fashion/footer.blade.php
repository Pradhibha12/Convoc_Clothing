...
@php
    $footer_about_text = get_frontend_settings('footer_about_text') ?? 'Convoc was born out of a simple yet profound idea – to provide high-quality promotional T-shirts at the best price.';
    $footer_copyright_text = get_frontend_settings('footer_copyright_text') ?? 'Copyright © 2026 All Rights Reserved Designed with ❤️ by Wspl Hosting Solution (OPC) Private Limited';
    $footer_quick_links = get_frontend_settings('footer_quick_links', true);
    $footer_support_links = get_frontend_settings('footer_support_links', true);
    $footer_quick_links_title = get_frontend_settings('footer_quick_links_title') ?? 'Quick Links';
    $footer_top_categories_title = get_frontend_settings('footer_top_categories_title') ?? 'Top Categories';
    $footer_support_title = get_frontend_settings('footer_support_title') ?? 'Support';
    $footer_contact_title = get_frontend_settings('footer_contact_title') ?? 'Contact Us';
@endphp
                    ...
                    <div class="col-lg-4">
                        <div class="footer-top-area mb-30px pb-30px d-flex align-items-end justify-content-between gap-4 flex-wrap">
                            <div class="">
                                ...
                                <p class="al-subtitle2-16px max-w-337px builder-editable" builder-identity="1">{!! nl2br(e($footer_about_text)) !!}</p>
                            </div>
                        </div>
                    </div>
                    <div class="col-lg-8">
                        <div class="footer-middle-area d-flex justify-content-between flex-wrap">
                            <div class="footer-nav-area">
                                <h4 class="al-title-18px text-white mb-4 builder-editable" builder-identity="2">{{ get_phrase($footer_quick_links_title) }}</h4>
                                <ul class="d-flex flex-column gap-18px">
                                    @if(!empty($footer_quick_links) && is_array($footer_quick_links))
                                        @foreach($footer_quick_links as $q_link)
                                            @php
                                                $clean_q_url = str_replace('\\', '', $q_link['url'] ?? '');
                                                $q_target_url = str_starts_with($clean_q_url, 'http') || str_starts_with($clean_q_url, '/') ? url($clean_q_url) : url('/' . $clean_q_url);
                                            @endphp
                                            <li><a href="{{ $q_target_url }}" class="footer-nav-link">{{ get_phrase($q_link['title'] ?? '') }}</a></li>
                                        @endforeach
                                    @else
                            ...
                            @endphp
                            <div class="footer-nav-area">
                                <h4 class="al-title-18px text-white mb-4 builder-editable" builder-identity="3">{{ get_phrase($footer_top_categories_title) }}</h4>
                                <ul class="d-flex flex-column gap-18px">
                            ...
                            <div class="footer-nav-area">
                                <h4 class="al-title-18px text-white mb-4 builder-editable" builder-identity="4">{{ get_phrase($footer_support_title) }}</h4>
                                <ul class="d-flex flex-column gap-18px">
                                    @if(!empty($footer_support_links) && is_array($footer_support_links))
                                        @foreach($footer_support_links as $s_link)
                                            @php
                                                $clean_s_url = str_replace('\\', '', $s_link['url'] ?? '');
                                                $s_target_url = str_starts_with($clean_s_url, 'http') || str_starts_with($clean_s_url, '/') ? url($clean_s_url) : url('/' . $clean_s_url);
                                            @endphp
                                            <li><a href="{{ $s_target_url }}" class="footer-nav-link">{{ get_phrase($s_link['title'] ?? '') }}</a></li>
                                        @endforeach
                                    @else
        ...
        <div class="row">
            <div class="col-lg-12">
                <div class="footer-copyright-area d-flex align-items-center justify-content-center justify-content-sm-between column-gap-4 row-gap-4 flex-wrap flex-column flex-sm-row">
                    <p class="al-subtitle2-16px lh-1  text-center text-sm-left builder-editable" builder-identity="6">{{ get_phrase($footer_copyright_text) }}</p>
