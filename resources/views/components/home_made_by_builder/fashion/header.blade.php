             ...
             <div class="col-auto col-lg order-2 order-lg-1">
              ...
              <div class="col-auto col-lg-6 order-1 order-lg-2">
                  <div class="d-lg-flex justify-content-center d-none">
                      @php
                          $header_menu_items = get_frontend_settings('header_menu_items', true);
                      @endphp
                      <nav>
                          <ul class="header-menu-ul">
                              @if(!empty($header_menu_items) && is_array($header_menu_items))
                                  @foreach($header_menu_items as $m_item)
                                      @if(($m_item['type'] ?? '') == 'category_dropdown')
                                          <li class="mega-menu-btn-wrap">
                                              <a href="javascript:void(0)" class="header-menuitem-a mega-menu-btn">
                                                  <span>{{ get_phrase($m_item['title'] ?? 'Categories') }}</span>
                                                              ...
                                                              <div class="mega-list-ads-wrap d-flex gap-3 align-items-start">
                                                                  ...
                                                                  <div class="mega-ads-wrap">
                                                                      @foreach ($categories->take(1) as $index => $category)
                                                                          @if (!empty($category->thumbnail) && $index === 0)
                                      ...
                                      @else
                                           @php
                                               $clean_url = str_replace('\\', '', $m_item['url'] ?? '');
                                               $target_url = ($m_item['type'] ?? '') == 'route' && Route::has($m_item['route'] ?? '') 
                                                   ? route($m_item['route']) 
                                                   : (str_starts_with($clean_url, 'http') || str_starts_with($clean_url, '/') ? url($clean_url) : url('/' . $clean_url));
                                           @endphp
                                           <li>
                                               <a href="{{ $target_url }}" target="{{ $m_item['target'] ?? '_self' }}" class="header-menuitem-a @if (request()->is(trim($clean_url, '/'))) active @endif">{{ get_phrase($m_item['title'] ?? '') }}</a>
                                           </li>
                                      @endif
                                  @endforeach
                              @else
