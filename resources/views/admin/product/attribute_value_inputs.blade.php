...
@if($attr_type && $attr_type->attributes->count() > 0)
    <div class="d-flex flex-wrap align-items-center gap-2 p-3 bg-light border rounded-3">
        ...
        <div class="form-check me-3 mb-2 p-2 px-3 border border-primary rounded bg-white shadow-sm cursor-pointer d-flex align-items-center attribute-checkbox-card" onclick="var cb = this.querySelector('input'); if(cb && event.target !== cb) { cb.click(); event.preventDefault(); }">
            <input class="form-check-input ms-0 me-2 cursor-pointer" type="checkbox" id="select_all_{{ $attribute_type_id }}_{{ isset($product_id) ? $product_id : 'new' }}" onchange="toggleSelectAllAttributes('{{ $attribute_type_id }}', '{{ isset($product_id) ? $product_id : 'new' }}', this.checked)">
            <label class="form-check-label fw-bold text-primary cursor-pointer mb-0" for="select_all_{{ $attribute_type_id }}_{{ isset($product_id) ? $product_id : 'new' }}">
                Select All
            </label>
        </div>
        ...
        @foreach ($attributes as $attribute)
            @php
                ...
                $chk_id = "attr_check_" . $attribute->id . "_" . (isset($product_id) ? $product_id : 'new');
            @endphp

            <div class="form-check me-3 mb-2 p-2 px-3 border rounded bg-white shadow-sm cursor-pointer d-flex align-items-center attribute-checkbox-card" onclick="var cb = this.querySelector('input'); if(cb && event.target !== cb) { cb.click(); event.preventDefault(); }">
                <input class="form-check-input ms-0 me-2 cursor-pointer" type="checkbox" name="product_attributes[{{ $attribute_type_id }}][{{ $attribute->id }}]" value="1" id="{{ $chk_id }}" {{ $is_checked ? 'checked' : '' }}>
                <label class="form-check-label fw-medium text-dark cursor-pointer mb-0" for="{{ $chk_id }}">
                    {{ $attribute->name }}
                </label>
            </div>
        @endforeach
...
<style>
    .attribute-checkbox-card:has(input:checked) {
...
@once
<script>
    function toggleSelectAllAttributes(typeId, prodId, checked) {
        const selector = `input[name^="product_attributes[${typeId}]"]`;
