$(document).ready(function () {
    ...
    // Image zoom plugin code (Desktop only)
    if ($('.image-zoom-active').length > 0 && !('ontouchstart' in window) && $(window).width() >= 992) {
    ...
    if ($('.products-slider').length > 0) {
        (function () {
            ...
            if (prevButton) {
                prevButton.addEventListener('click', () => shoppingcart.slidePrev());
            }
            if (nextButton) {
                nextButton.addEventListener('click', () => shoppingcart.slideNext());
            }
        })();
    }
